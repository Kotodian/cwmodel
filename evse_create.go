...
func (ec *EvseCreate) check() error {
	if _, ok := ec.mutation.Version(); !ok {
		return &ValidationError{Name: "version", err: errors.New(`cwmodel: missing required field "Evse.version"`)}
	}
	if _, ok := ec.mutation.CreatedBy(); !ok {
		return &ValidationError{Name: "created_by", err: errors.New(`cwmodel: missing required field "Evse.created_by"`)}
	}
	if _, ok := ec.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`cwmodel: missing required field "Evse.created_at"`)}
	}
	if _, ok := ec.mutation.UpdatedBy(); !ok {
		return &ValidationError{Name: "updated_by", err: errors.New(`cwmodel: missing required field "Evse.updated_by"`)}
	}
	if _, ok := ec.mutation.UpdatedAt(); !ok {
	...
	if _, ok := ec.mutation.ConnectorNumber(); !ok {
		return &ValidationError{Name: "connector_number", err: errors.New(`cwmodel: missing required field "Evse.connector_number"`)}
	}
	if _, ok := ec.mutation.EquipmentID(); !ok {
		return &ValidationError{Name: "equipment", err: errors.New(`cwmodel: missing required edge "Evse.equipment"`)}
	}
	return nil
}
