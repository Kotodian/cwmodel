...
func (amic *AppModuleInfoCreate) check() error {
	if _, ok := amic.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`cwmodel: missing required field "AppModuleInfo.name"`)}
	}
	if _, ok := amic.mutation.Desc(); !ok {
		return &ValidationError{Name: "desc", err: errors.New(`cwmodel: missing required field "AppModuleInfo.desc"`)}
	}
	return nil
}
