...
type EquipmentFirmwareEffect struct {
	...
	equipment_equipment_firmware_effect *int
	firmware_equipment_firmware_effect  *int
}
...
func (*EquipmentFirmwareEffect) scanValues(columns []string) ([]interface{}, error) {
	values := make([]interface{}, len(columns))
	for i := range columns {
		switch columns[i] {
		case equipmentfirmwareeffect.FieldID, equipmentfirmwareeffect.FieldRequestID, equipmentfirmwareeffect.FieldState:
			values[i] = new(sql.NullInt64)
		case equipmentfirmwareeffect.ForeignKeys[0]: // equipment_equipment_firmware_effect
			values[i] = new(sql.NullInt64)
		case equipmentfirmwareeffect.ForeignKeys[1]: // firmware_equipment_firmware_effect
			values[i] = new(sql.NullInt64)
		default:
...
func (efe *EquipmentFirmwareEffect) assignValues(columns []string, values []interface{}) error {
	...
	for i := range columns {
		...
		case equipmentfirmwareeffect.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field equipment_equipment_firmware_effect", value)
			} else if value.Valid {
				efe.equipment_equipment_firmware_effect = new(int)
				*efe.equipment_equipment_firmware_effect = int(value.Int64)
			}
		case equipmentfirmwareeffect.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field firmware_equipment_firmware_effect", value)
			} else if value.Valid {
				efe.firmware_equipment_firmware_effect = new(int)
				*efe.firmware_equipment_firmware_effect = int(value.Int64)
			}
		}
	}
	return nil
...
func (efe *EquipmentFirmwareEffect) Unwrap() *EquipmentFirmwareEffect {
	tx, ok := efe.config.driver.(*txDriver)
	if !ok {
		panic("ent: EquipmentFirmwareEffect is not a transactional entity")
	}
	efe.config.driver = tx.drv
	return efe
}
