...
//		GroupBy(orderinfo.FieldRemoteStartID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.OrderInfo.Query().
//		Select(orderinfo.FieldRemoteStartID).
//		Scan(ctx, &v)
...
func (oiq *OrderInfoQuery) sqlAll(ctx context.Context) ([]*OrderInfo, error) {
	...
	if query := oiq.withConnector; query != nil {
		...
		for i := range nodes {
			if nodes[i].connector_order_info == nil {
				continue
			}
			fk := *nodes[i].connector_order_info
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "connector_order_info" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oiq.withEquipment; query != nil {
		...
		nodeids := make(map[int][]*OrderInfo)
		for i := range nodes {
			if nodes[i].equipment_order_info == nil {
				continue
			}
			fk := *nodes[i].equipment_order_info
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "equipment_order_info" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := oiq.withOrderEvent; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.OrderEvent = []*OrderEvent{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "order_info_order_event" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "order_info_order_event" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.OrderEvent = append(node.Edges.OrderEvent, n)
		}
	}
