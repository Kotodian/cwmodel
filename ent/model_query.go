...
func (mq *ModelQuery) QueryEquipment() *EquipmentQuery {
...
func (mq *ModelQuery) WithEquipment(opts ...func(*EquipmentQuery)) *ModelQuery {
	query := &EquipmentQuery{config: mq.config}
...
func (mq *ModelQuery) sqlAll(ctx context.Context) ([]*Model, error) {
	...
	if query := mq.withEquipment; query != nil {
		...
		query.Where(predicate.Equipment(func(s *sql.Selector) {
			s.Where(sql.InValues(model.EquipmentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.model_equipment
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "model_equipment" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "model_equipment" returned %v for node %v`, *fk, n.ID)
			}
