...
func (*AppModuleInfo) scanValues(columns []string) ([]interface{}, error) {
...
func (ami *AppModuleInfo) assignValues(columns []string, values []interface{}) error {
...
func (ami *AppModuleInfo) Unwrap() *AppModuleInfo {
	tx, ok := ami.config.driver.(*txDriver)
	if !ok {
		panic("ent: AppModuleInfo is not a transactional entity")
	}
	ami.config.driver = tx.drv
	return ami
}
