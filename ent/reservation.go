...
type Reservation struct {
	...
	// AuthorizationMode holds the value of the "authorization_mode" field.
	// 授权模式
	...
	// Additional holds the value of the "additional" field.
	// 额外授权id
	...
	connector_reservation *int
	equipment_reservation *int
}
...
func (*Reservation) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case reservation.ForeignKeys[0]: // connector_reservation
			values[i] = new(sql.NullInt64)
		case reservation.ForeignKeys[1]: // equipment_reservation
			values[i] = new(sql.NullInt64)
		default:
...
func (r *Reservation) assignValues(columns []string, values []interface{}) error {
	...
	for i := range columns {
		...
		case reservation.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field connector_reservation", value)
			} else if value.Valid {
				r.connector_reservation = new(int)
				*r.connector_reservation = int(value.Int64)
			}
		case reservation.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field equipment_reservation", value)
			} else if value.Valid {
				r.equipment_reservation = new(int)
				*r.equipment_reservation = int(value.Int64)
			}
		}
	}
	return nil
