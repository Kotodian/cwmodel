...
func (cc *ConnectorCreate) check() error {
	...
	if _, ok := cc.mutation.EvseSerial(); !ok {
		return &ValidationError{Name: "evse_serial", err: errors.New(`ent: missing required field "Connector.evse_serial"`)}
	}
	if _, ok := cc.mutation.Serial(); !ok {
		return &ValidationError{Name: "serial", err: errors.New(`ent: missing required field "Connector.serial"`)}
	}
	...
	if _, ok := cc.mutation.EquipmentID(); !ok {
		return &ValidationError{Name: "equipment", err: errors.New(`ent: missing required edge "Connector.equipment"`)}
	}
	return nil
}
