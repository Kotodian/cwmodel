...
func (mq *ManufacturerQuery) QueryEquipment() *EquipmentQuery {
	query := &EquipmentQuery{config: mq.config}
...
func (mq *ManufacturerQuery) Clone() *ManufacturerQuery {
	...
	return &ManufacturerQuery{
		...
		withEquipment: mq.withEquipment.Clone(),
...
func (mq *ManufacturerQuery) WithEquipment(opts ...func(*EquipmentQuery)) *ManufacturerQuery {
	query := &EquipmentQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withEquipment = query
	return mq
}
...
func (mq *ManufacturerQuery) sqlAll(ctx context.Context) ([]*Manufacturer, error) {
	...
	if query := mq.withEquipment; query != nil {
		...
		query.Where(predicate.Equipment(func(s *sql.Selector) {
			s.Where(sql.InValues(manufacturer.EquipmentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.manufacturer_equipment
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "manufacturer_equipment" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "manufacturer_equipment" returned %v for node %v`, *fk, n.ID)
			}
