...
type Firmware struct {
	...
	// EquipVersion holds the value of the "equip_version" field.
	// 固件版本
...
func (e FirmwareEdges) EquipmentOrErr() ([]*Equipment, error) {
	if e.loadedTypes[0] {
...
func (*Firmware) scanValues(columns []string) ([]interface{}, error) {
...
func (f *Firmware) assignValues(columns []string, values []interface{}) error {
...
// QueryEquipment queries the "equipment" edge of the Firmware entity.
func (f *Firmware) QueryEquipment() *EquipmentQuery {
	return (&FirmwareClient{config: f.config}).QueryEquipment(f)
}
