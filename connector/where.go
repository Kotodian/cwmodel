...
func EquipmentID(v datasource.UUID) predicate.Connector {
	vc := uint64(v)
	return predicate.Connector(func(s *sql.Selector) {
...
func EquipmentIDEQ(v datasource.UUID) predicate.Connector {
	vc := uint64(v)
	return predicate.Connector(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldEquipmentID), vc))
	})
...
func EquipmentIDNEQ(v datasource.UUID) predicate.Connector {
	vc := uint64(v)
	return predicate.Connector(func(s *sql.Selector) {
...
func EquipmentIDIn(vs ...datasource.UUID) predicate.Connector {
	v := make([]any, len(vs))
	for i := range v {
		v[i] = uint64(vs[i])
	}
...
func EquipmentIDNotIn(vs ...datasource.UUID) predicate.Connector {
	v := make([]any, len(vs))
	for i := range v {
		v[i] = uint64(vs[i])
	}
...
func HasSmartChargingEffect() predicate.Connector {
	return predicate.Connector(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
func HasSmartChargingEffectWith(preds ...predicate.SmartChargingEffect) predicate.Connector {
	return predicate.Connector(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
